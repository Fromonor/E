	...
	The main explorer interface
		...
		local function main()
			...
			addObject = function(root)
				...
				for i = 1,#insts do if i % 250 == 0 then task.wait() end -- PATCHED: yield in large loop
					local obj = insts[i]
				...
				if not updateDebounce and Explorer.IsNodeVisible(par) then
					if expanded[par] then
						-- Explorer.PerformUpdate() -- PATCHED: prevent auto update
					elseif not refreshDebounce then
						-- Explorer.PerformRefresh() -- PATCHED: prevent auto refresh
					end
				end
			end
			...
			-- PATCHED: Disable SetupConnections
			...
			Explorer.InitDelCleaner = function()
				coroutine.wrap(function()
					...
					while true do
						...
						if processed and not refreshDebounce then -- Explorer.PerformRefresh() -- PATCHED: prevent auto refresh end
						fw(0.5)
					end
				end)()
			end
