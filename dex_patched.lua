...
insts = {table.unpack(insts, 1, math.min(#insts, 1000))} -- PATCHED: limit to 1000
				for i = 1,#insts do
					local obj = insts[i]
			...
			Explorer.Update = function()
				...
				-- Nil Instances
				if false then -- PATCHED: disabled nil instance tracking
			...
			Explorer.SetupConnections = function()
				...
				if Main.Elevated then
					-- descendantAddedCon = game.DescendantAdded:Connect(addObject) -- PATCHED
					-- descendantRemovingCon = game.DescendantRemoving:Connect(removeObject) -- PATCHED
				else
